...
for i, m in enumerate(sorted(ret, key=lambda t: t['发言用户数'], reverse=reverse_dialog)[:limit_dialog]):
    print(i+1, '\t', m['发言用户数'], '\t', m['消息数'], '\t', m['_id'], '\t', dialogs_id_title[m['_id']])
print()

print('='*15, '排序: 消息数', '='*15)
print('排序\t消息数\t发言用户数\tdialog_id\t群/频道名称')
for i, m in enumerate(sorted(ret, key=lambda t: t['消息数'], reverse=reverse_dialog)[:limit_dialog]):
